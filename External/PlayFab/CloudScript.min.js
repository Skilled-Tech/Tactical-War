"use strict";handlers.ClaimDailyReward=function(args,context){let template=API.DailyReward.Template.Retrieve();let playerData=API.DailyReward.PlayerData.Retrieve(currentPlayerId);let reward=null;if(playerData==null)
...
let result=new API.DailyReward.Result(progress,itemIDs);return result;};handlers.FinishLevel=function(args){if(args==null){log.error("no arguments specified");return;}
...
let result={rewards:rewardsIDs};return result;};handlers.UpgradeItem=function(args){if(args==null){log.error("no arguments specified");return;}
...
let rank=template.element.ranks[instanceData.element.value];if(rank==null){log.error("rank of index "+instanceData.element.value+" can't be null");return;}
if(inventory.CompliesWith(rank.requirements)==false){log.error("inventory doesn't comply with upgrade requirements");return;}
PlayFab.Player.Currency.Subtract(currentPlayerId,rank.cost.type,rank.cost.value);PlayFab.Player.Inventory.ConsumeAll(inventory,rank.requirements);instanceData.element.Increment();API.Upgrades.InstanceData.Save(currentPlayerId,itemInstance,instanceData.data);let result=new API.Upgrades.Result(true);return result;};handlers.Reward=function(args){if(args==null){log.error("no arguments specified");return;}
PlayFab.Catalog.Item.GrantAll(currentPlayerId,args,"Reward");};var Sandbox;(function(Sandbox){function Call(){}})(Sandbox||(Sandbox={}));var MyJSON;(function(MyJSON){MyJSON.IgnoreCharacter='$';function Write(object){function Replacer(key,value){if(key[0]==MyJSON.IgnoreCharacter)
...
Class.WriteProperty=WriteProperty;})(Class=Utility.Class||(Utility.Class={}));})(Utility||(Utility={}));var API;(function(API){class DropTable{constructor(ID,iterations){this.ID=ID;this.iterations=iterations;}}
...
API.ItemStack=ItemStack;let Difficulty;(function(Difficulty){Difficulty[Difficulty["Normal"]=1]="Normal";Difficulty[Difficulty["Hard"]=2]="Hard";Difficulty[Difficulty["Skilled"]=3]="Skilled";})(Difficulty=API.Difficulty||(API.Difficulty={}));})(API||(API={}));var API;(function(API){let DailyReward;(function(DailyReward){DailyReward.ID="daily-rewards";class Result{constructor(progress,items){this.progress=progress;this.items=items;}}
...
return null;let instance=MyJSON.Read(PlayerData,json);return instance;}
...
DailyReward.Template=Template;})(DailyReward=API.DailyReward||(API.DailyReward={}));})(API||(API={}));var API;(function(API){class Reward{constructor(items,droptable){this.items=items;this.droptable=droptable;}
...
API.Reward=Reward;})(API||(API={}));var API;(function(API){let Upgrades;(function(Upgrades){Upgrades.ID="upgrades";class Result{constructor(success){this.success=success;}}
...
toJSON(){return MyJSON.Write(this.list);}
...
Upgrades.ItemData=ItemData;})(Upgrades=API.Upgrades||(API.Upgrades={}));})(API||(API={}));var API;(function(API){let Upgrades;(function(Upgrades){class Template{constructor(object){this.name=object.name;this.elements=[];for(let i=0;i<object.elements.length;i++){let instance=new Template.Element(this,i,object.elements[i]);this.elements.push(instance);}}
...
Template.GetAll=GetAll;class Element{constructor($template,$index,source){this.$template=$template;this.$index=$index;this.type=source.type;this.ranks=[];for(let i=0;i<source.ranks.length;i++){let instance=new Element.Rank(this,i,source.ranks[i]);this.ranks.push(instance);}}
get template(){return this.$template;}
get index(){return this.$index;}}
Template.Element=Element;(function(Element){class Rank{constructor($element,$index,source){this.$element=$element;this.$index=$index;this.cost=source.cost;this.percentage=source.percentage;this.requirements=source.requirements;}
get element(){return this.$element;}
get index(){return this.$index;}
get previous(){if(this.index==0)
return null;return this.$element.ranks[this.index-1];}
get next(){if(this.index+1==this.element.ranks.length)
return null;return this.element.ranks[this.index+1];}
get isFirst(){return this.index==0;}
get isLast(){return this.index+1>=this.element.ranks.length;}}
Element.Rank=Rank;})(Element=Template.Element||(Template.Element={}));class Snapshot{constructor(data,element){this.data=data;this.element=element;}
...
Template.Snapshot=Snapshot;})(Template=Upgrades.Template||(Upgrades.Template={}));})(Upgrades=API.Upgrades||(API.Upgrades={}));})(API||(API={}));var API;(function(API){let World;(function(World){World.ID="world";let Level;(function(Level){let Finish;(function(Finish){let Occurrence;(function(Occurrence){Occurrence[Occurrence["Initial"]=1]="Initial";Occurrence[Occurrence["Recurring"]=2]="Recurring";})(Occurrence=Finish.Occurrence||(Finish.Occurrence={}));class Result{constructor(reward){this.rewards=reward;}}
...
return null;var instance=MyJSON.Read(PlayerData,json);return instance;}
static Create(){let source={regions:[]};var instance=new PlayerData(source);return instance;}
static Save(playerID,data){let json=MyJSON.Write(data);PlayFab.Player.Data.ReadOnly.Write(playerID,API.World.ID,json);}}
World.PlayerData=PlayerData;(function(PlayerData){class Region{constructor($playerData,$index,source){this.$playerData=$playerData;this.$index=$index;this.name=source.name;this.progress=source.progress;this.difficulty=source.difficulty;}
Increment(){this.progress+=1;}
get playerData(){return this.$playerData;}
get index(){return this.$index;}
static Create(playerData,index,name,progress){let source={name:name,progress:progress,difficulty:API.Difficulty.Normal};var instance=new Region(playerData,index,source);return instance;}}
PlayerData.Region=Region;class Snapshot{constructor(data,region,occurrence){this.data=data;this.region=region;this.occurrence=occurrence;}
...
let region=data.Find(args.region);if(region==null){if(template.region.previous==null)
{}
else{var previous=data.Find(template.region.previous.name);if(previous==null)
throw("trying to index region "+args.region+" without unlocking the previous region: "+template.region.previous.name);if(previous.progress<template.region.previous.size)
...
{if(region.difficulty+1==args.difficulty){if(region.progress<template.region.size)
throw("can't progress difficulty, region not completed at difficulty "+region.difficulty+" yet");region.difficulty=args.difficulty;region.progress=0;}
else
{throw("can't change difficulty from "+region.difficulty+" to "+args.difficulty);}}
if(args.level>region.progress){if(args.difficulty>=region.difficulty){throw("trying to complete level of index "+args.level+" without completing the previous levels");}}
let occurrence;if(region.progress==args.level&&args.difficulty==region.difficulty)
occurrence=World.Level.Finish.Occurrence.Initial;else
occurrence=World.Level.Finish.Occurrence.Recurring;return new Snapshot(data,region,occurrence);}}
...
throw("no World Template data defined within PlayFab Title Data");var instance=MyJSON.Read(Template,json);return instance;}
...
Template.Region=Region;(function(Region){class Level{constructor($region,$index,source){this.$region=$region;this.$index=$index;this.rewards=[];for(let i=0;i<source.rewards.length;i++){let instance=new Level.Reward(source.rewards[i]);this.rewards.push(instance);}}
...
Region.Level=Level;(function(Level){class Reward{constructor(source){this.data=source.data;if(source.requirements==null)
...
Level.Reward=Reward;(function(Reward){class Requirements{constructor(source){this.occurrence=source.occurrence;this.difficulty=source.difficulty;}
IsValidOccurrence(object){if(this.occurrence==null)
return true;return this.occurrence.indexOf(object)>0;}
IsValidDifficulty(object){if(this.difficulty==null)
return true;return this.difficulty.indexOf(object)>0;}
CompliesWith(args,playerData){if(this.occurrence!=null&&this.occurrence.indexOf(playerData.occurrence)<0)
return false;if(this.difficulty!=null&&this.difficulty.indexOf(args.difficulty)<0)
...
return this.items[i];return null;}
CompliesWith(requirements){if(requirements==null||requirements.length==0)
return true;for(let i=0;i<requirements.length;i++){let instance=this.FindWithID(requirements[i].item);if(instance==null)
return false;if(instance.RemainingUses==null){if(requirements[i].count>1)
return false;else
continue;}
