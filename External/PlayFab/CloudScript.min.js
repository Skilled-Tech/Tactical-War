...
let rank={requirements:template.element.requirements[instanceData.element.value],cost:{value:template.element.cost.Calculate(instanceData.element.value),type:"GD",}};log.info("rank:"+MyJSON.Stringfy(rank));if(inventory.CompliesWith(rank.requirements)==false){log.error("inventory doesn't comply with upgrade requirements");return;}
...
function WelcomeNewPlayer(args){var inventory=PlayFab.Player.Inventory.Retrieve(currentPlayerId);var token="New_Player_Reward";if(inventory.Contains(token)){log.error("player "+currentPlayerId+" has already been rewarded a "+token);return;}
var rewards=[token,"Hero","Meteor_Shower"];PlayFab.Catalog.Item.GrantAll(currentPlayerId,rewards,"New Player Welcome Reward");var result={rewards:rewards,};return result;}
...
Class.WriteProperty=WriteProperty;})(Class=Utility.Class||(Utility.Class={}));})(Utility||(Utility={}));function IsOnPlayFab(){return globalThis.handlers!=null;}
...
static FromString(text){let start=text.indexOf('[');if(start<0)
...
Calculate(i){return this.initial+(this.multiplier*i);}
static Create(object){return new FactorialValue(object.initial,object.multiplier);}}
...
Template.GetAll=GetAll;class Element{constructor($template,$index,source){this.$template=$template;this.$index=$index;this.type=source.type;this.cost=API.FactorialValue.Create(source.cost);this.percentage=API.FactorialValue.Create(source.percentage);this.requirements=[];for(let x=0;x<source.requirements.length;x++){this.requirements.push([]);for(let y=0;y<source.requirements[x].length;y++){var stack=API.ItemStack.FromString(source.requirements[x][y]);this.requirements[x].push(stack);}}}
