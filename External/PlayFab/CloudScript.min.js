...
let result={rewards:rewardsIDs,stars:playerData.star==null?0:playerData.star.rank};return result;};handlers.UpgradeItem=function(args){if(args==null){log.error("no arguments specified");return;}
...
PlayFab.Catalog.Item.GrantAll(currentPlayerId,args,"Reward");};handlers.WelcomeNewPlayer=function(args){var inventory=PlayFab.Player.Inventory.Retrieve(currentPlayerId);var token="New_Player_Reward";if(inventory.Contains(token)){log.error("player "+currentPlayerId+" has already been rewarded a "+token);return;}
var rewards=[token,"Savage","Archer","Meteor_Shower"];PlayFab.Catalog.Item.GrantAll(currentPlayerId,rewards,"New Player Welcome Reward");var result={rewards:rewards,};return result;};var Sandbox;(function(Sandbox){function Call(){}})(Sandbox||(Sandbox={}));var MyJSON;(function(MyJSON){MyJSON.IgnoreCharacter='$';function Write(object){function Replacer(key,value){if(key[0]==MyJSON.IgnoreCharacter)
